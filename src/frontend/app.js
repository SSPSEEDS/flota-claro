...
async function cargarFiltros() {
  ...
  selPlan.innerHTML = '<option value="">Todos</option>' +
    r.planes.map(p => `<option value="${p}">${p}</option>`).join('');
}
...
function renderPivote(lineas) {
  if (!lineas.length) { mostrarVacio(); return; }
  const periodos = [...new Set(lineas.map(l => l.periodo))].sort().reverse();
  ...
  const totalesMes = {};
  periodos.forEach(p => totalesMes[p] = 0);

  const filas = [...porLinea.entries()].map(([linea, e]) => {
    const celdas = periodos.map(p => {
      const v = e.datos[p];
      if (typeof v === 'number') totalesMes[p] += v;
      return `<td>${money(v)}</td>`;
    }).join('');
    return `<tr><td class="txt">${e.usuario || ''}</td><td class="txt">${linea}</td>${celdas}</tr>`;
  }).join('');

  const totalRow = `<tr class="total-row"><td class="txt" colspan="2">TOTAL</td>` +
    periodos.map(p => `<td>${money(totalesMes[p])}</td>`).join('') + '</tr>';

  $('#resumen').innerHTML = `Vista por mes · <b>${porLinea.size}</b> líneas × <b>${periodos.length}</b> meses`;
  $('#tablaWrap').innerHTML =
    `<table><thead><tr><th>Usuario</th><th>Línea</th>${periodos.map(p => `<th>${p}</th>`).join('')}</tr></thead>` +
    `<tbody>${filas}${totalRow}</tbody></table>`;
}
